...
contract("CourseMarketplace", accounts => {
	...
	describe("Purchase the new Course", () => {
		...
		it("should NOT allowe  to repurchase allredy owned course", async () => {
	...
	describe("Transfer ownership", () => {
		...
		it("getContractOwner should return deployer address", async() => {
			assert.equal(
				contractOwner,
				currentOwner,
				"Contract owner is not matching the one from getContractOwner functions")
		})
		it("Shold NOT transfer ownersip when contract owner is not sending TX", async () => {
	...
	describe("Repurchase course", () => {
		...
		it("should NOT repurchase when the course doesn't exist", async () => {
			const notExistingHash = "0x3a7369da1661b5b81083c425db9a54a757b3e9550cc9d2cca1a48c4e98cb7e7a"
		...
		it("should be abble repurchase with original buyer", async () => {
			const beforeTxBuyerBalance = await getBalance(buyer)
			const beforeTxContractBalance = await getBalance(_contract.address)
			...
			assert.equal(
				...
				afterTxBuyerBalance,
				"Client ballance is not correct!"
			)

			assert.equal(
				toBN(beforeTxContractBalance).add(toBN(value)),
				afterTxContractBalance,
				"Contract ballance is not correct!"
			)
		})
	...
	describe("Normal withdraw", () => {
		...
		const overLimitFunds = "10000000000000000000"
		...
		it("should faild when withdrawing address is NOT owner", async () => {
			await catchRevert(_contract.withdraw(fundsToDeposit, {from: buyer}))
		})

		it("should faild when withdrawing value is overLimited", async () => {
			await catchRevert(_contract.withdraw(overLimitFunds, {from: contractOwner}))
		})

		it("should have +1 eth after withdraw", async () => {
	...
	describe("Emergency withdraw", () => {
		...
		it("should fail when contract is NOT stooped", async () => {
